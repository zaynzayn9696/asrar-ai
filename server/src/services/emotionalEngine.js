...
function getDialectGuidance(language, dialect) {
  ...
  if (lang === 'en') {
    return 'Language: reply in natural, clear English. If the user occasionally mixes Arabic, you may briefly mirror that, but keep your main reply in English.';
  }

  const key = String(dialect || 'msa').toLowerCase();

  const map = {
    msa: 'اللغة: استخدم عربية فصحى حديثة بسيطة وواضحة. يمكنك تقريب التعبير من أسلوب المستخدم، لكن تجنّب المبالغة في اللهجات ما لم يكتب المستخدم بوضوح بلهجة معيّنة.',
    jo: 'اللغة: استخدم لهجة أردنية طبيعية في معظم ردّك (كلام يومي بسيط)، وتجنّب الرجوع لعربية فصحى رسمية إلّا إذا كتب المستخدم بهذه الطريقة.',
    sy: 'اللغة: استخدم لهجة شامية/سورية بسيطة وطبيعية في معظم الجمل، وتجنّب الفصحى الرسمية ما أمكن إلّا إذا كان المستخدم يكتب بها.',
    lb: 'اللغة: استخدم لهجة لبنانية طبيعية ومألوفة، مع صياغة يومية وليست فصحى رسمية، إلّا إذا كتب المستخدم بصياغة رسمية صريحة.',
    ps: 'اللغة: استخدم لهجة فلسطينية طبيعية قدر الإمكان، مع تجنّب العودة للفصحى الرسمية إلا إذا كانت طريقة كتابة المستخدم رسمية تماماً.',
    iq: 'اللغة: استخدم لهجة عراقية طبيعية في الكلام اليومي، ولا تعُد إلى الفصحى الرسمية إلا إذا كان المستخدم يكتب بها فقط.',
    eg: 'اللغة: استخدم لهجة مصرية (عامية مصرية) واضحة ومفهومة في معظم ردّك، وتجنّب الفصحى الرسمية إلا عند الضرورة أو إذا كتب المستخدم بها.',
    sa: 'اللغة: استخدم لهجة خليجية/سعودية طبيعية في الكلام اليومي، وتجنّب الفصحى الرسمية قدر الإمكان إلا إذا كتب المستخدم بها.',
    ae: 'اللغة: استخدم لهجة خليجية/إماراتية بسيطة وقريبة من كلام الناس اليومي، ولا تعتمد على الفصحى الرسمية إلا إذا كانت صياغة المستخدم رسمية بوضوح.',
    kw: 'اللغة: استخدم لهجة خليجية/كويتية طبيعية في الحديث، مع تجنّب الرجوع للفصحى الرسمية إلا إذا كانت طريقة كتابة المستخدم رسمية تماماً.',
    bh: 'اللغة: استخدم لهجة خليجية/بحرينية طبيعية قدر الإمكان، وتجنّب الفصحى الرسمية ما لم يكتب المستخدم بها.',
    om: 'اللغة: استخدم لهجة خليجية/عُمانية بسيطة ومفهومة في معظم الوقت، مع تجنّب الفصحى الرسمية إلا إذا كان المستخدم يفضّلها بوضوح.',
    ye: 'اللغة: استخدم لهجة يمنية طبيعية في الكلام اليومي، وابتعد عن الفصحى الرسمية إلا عند الضرورة أو إذا كانت طريقة المستخدم رسمية تماماً.',
    ma: 'اللغة: استخدم لهجة مغربية (دارجة مغربية) في الكلام اليومي ما أمكن، وتجنّب الفصحى الرسمية إلا إذا كتب المستخدم بصياغة فصحى واضحة.',
    tn: 'اللغة: استخدم لهجة تونسية طبيعية في معظم الجمل، مع تجنّب الفصحى الرسمية قدر الإمكان إلا إذا كان المستخدم يكتب بها.',
  };

  const base = map[key] || map.msa;

  if (lang === 'mixed') {
    return base + '\nعند استخدام الإنجليزية، اجعلها إنجليزية طبيعية، لكن في المقاطع العربية استمرّ في استخدام هذه اللهجة وعدم العودة للفصحى الرسمية إلا إذا غيّر المستخدم أسلوبه.';
  }
